...
describe('Subdomain Migration', () => {
  ...
  const subDomainTestData: Array<[string, string, string, { found: number, migrated: number, collisions: number, pending: number }]> = [
    [
      ...
      { found: 1, migrated: 1, collisions: 0, pending: 0 } // expected output, successfully migrated
    ],
    [
      'sound idle panel often situate develop unit text design antenna vendor screen opinion balcony share trigger accuse scatter visa uniform brass update opinion media',
      'test2.id.stx', // Subdomain to be migrated
      'ST3Q2T3380WE1K5PW72R6R76Q8HRPEK8HR02W6V1M', // Owner mismatch
      { found: 1, migrated: 0, collisions: 0, pending: 0 } // expected output, not migrated due to owner mismatch
    ],
    [
      'sound idle panel often situate develop unit text design antenna vendor screen opinion balcony share trigger accuse scatter visa uniform brass update opinion media',
      'test3.id.stx', // Subdomain to be migrated, but collision detected
      'ST3WTH31TWVYDD1YGYKSZK8XFJ3Z1Z5JMGGRF4558', // Owner match
      { found: 1, migrated: 0, collisions: 1, pending: 0 } // expected output, name collision
    ]
  ];
  ...
  test.each(subDomainTestData)('Transfer subdomains to wallet-key addresses that correspond to all data-key addresses', async (mnemonic, subdomain, owner, expected) => {
    ...
    fetchMock
      ...
      .once(JSON.stringify({ txid: 'success' }));
  ...
  test('Subdomain signature verification', () => {
    ...
    // This follows the format of signature: [signature, publickey] according to above references
    subDomainOp.signature = [
      sig.data,
      compressPublicKey(publicKeyToString(pubKeyfromPrivKey(privateKey))).data.toString('hex'),
    ].join(',');

    // Verify that the generated signature is valid
    const [ signature, publicKey ] = subDomainOp.signature.split(',');
    // Skip the recovery params bytes from signature and then verify
    const isValid = verify(signature.slice(2), hash, publicKey);
